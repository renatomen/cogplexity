...
> **Status (2026-09-06):** implemented and shipped on `main`. This document is the decision record for that work. For scoring details the executable specification governs: the fixtures under `test/fixtures/spec/`, the calibration harness and ledger under `calibration/`, `README.md` and `PROVENANCE.md`. Where this text and those disagree, the tests are right; the amendments dated 2026-09-06 (R25, KTD9, KTD11, the Appendix A example under U2) record the differences found during execution.
...
- R25. A test compares the scoring function's per-file totals against the fixture and passes only on exact equality, and compares per-function findings against the fixture by location and score using the quantity SonarCloud reports per function (S3776): each function's own-body score, nested functions excluded and nesting counted from that body — not the inclusive root score the rule reports (KTD7), which the harness derives from the same increments; exceptions are files or spec clauses listed in a checked-in exception ledger, each entry carrying a one-line reason. (Amended 2026-09-06 after the first calibration: the corpus carries no S3776 issue although several roots exceed 15 inclusively, so the literal root-score comparison could never pass; the rule's inclusive number is kept as the better signal for the agent.)
